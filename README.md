...
- `lib/report.js` freezes the week's peg report on the first check after Friday's close (tk:rep:*) and sends it to Telegram subscribers (`/report` or `t.me/<bot>?start=rep`). `?p=report&w=YYYY-MM-DD` serves it to the /report page; while the week trades it is built live. Share card: `/api/og?p=report`.
- `lib/rights.js` holds what each issuer's tokens give the holder (legal form, custodian, dividends, who can mint and redeem, who can't hold them), summarized from the issuers' own documents with source links. It rides along in `/api/board` issuers and fills the "What you own" cards on every stock page, next to each version's live share ratio. Re-check the sources and bump `CHECKED` when editing.
