...
async function xget(path){
  let err;
  for (const h of XHOSTS){ try { return await getJson(h + path, {timeout: 6000}); } catch (e) { err = e; } }
  throw err;
}
...
// one call per token, so only for tokens that trade; each answer is kept six hours per server instance
const xRatios = symbols => pool(symbols, 10, sym => cached("xm:" + sym, 6 * 36e5,
  () => xget(`/public/assets/${encodeURIComponent(sym)}/multiplier?network=Solana`).then(j => num(j.currentMultiplier))).catch(() => null));
...
async function board(){
  ...
  if (xref) await within(pool(need, 8, async s => { const p = await xref.refPrice(xsym.get(s.ticker)); if (p > 0) quotes[s.ticker] = {bid: p, ask: p, at: new Date().toISOString()}; }), 5000);
