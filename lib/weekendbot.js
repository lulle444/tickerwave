...
// Send one text to every subscriber, once per week and kind. Drops chats that blocked the bot.
async function broadcast(week, kind, text){
  ...
  for (let i = 0; i < chats.length; i += 20){
    await Promise.all(chats.slice(i, i + 20).map(c => send(c, text, {disable_web_page_preview: true}).then(() => sent++).catch(async e => {
      if (/blocked|deactivated|chat not found|403/i.test(String(e.message))){ gone++; await unsubscribe(c); }
      else console.error("weekend send:", e.message);
    })));
  }
...
module.exports = {subscribe, unsubscribe, isSubscribed, signalText, resultText, maybeSendSignal, sendResult, SUBS};
