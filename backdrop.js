/* Animated background; its colors come from the brand variables in brand.css (the drifting light behind it is CSS, see body::before/::after): rolling swells, tide-chart lines that react to the pointer
...
const A = rgb("--glowA-rgb", "138,108,242"), B = rgb("--glowB-rgb", "75,63,214"), C = rgb("--glowC-rgb", "96,165,250");
const SWELLS = [
  {y: .80, amp: 22, len: 900, sp: .30, top: `rgba(${A},.10)`, bot: `rgba(${B},.16)`},
  {y: .86, amp: 18, len: 650, sp: -.42, top: `rgba(${C},.10)`, bot: `rgba(${B},.18)`},
  {y: .92, amp: 14, len: 480, sp: .55, top: `rgba(${A},.12)`, bot: `rgba(${B},.15)`},
];
let glints = [];
...
function lines(){
  ...
  for (let i = 0; i < n; i++){
    ...
    g.addColorStop(0, `rgba(${B},${a * .5})`);
    g.addColorStop(.15 + .7 * s, `rgba(${A},${a * 1.25})`);
    g.addColorStop(1, `rgba(${B},${a * .5})`);
